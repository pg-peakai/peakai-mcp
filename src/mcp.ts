...
const SERVER_INSTRUCTIONS = [
  ...
  "4. Get contacts:",
  "   - MANY leads → bulk_enrich(search_id, lead_ids, types) in ONE call. This is the efficient path — never loop lead_enrich for a list.",
  "   - ONE lead → lead_enrich(lead_id, types).",
  "",
  "RULES:",
  ...
  "- For a big list, collect lead_ids from lead_search (page through if needed) and call bulk_enrich in batches of ~50. It reveals rows concurrently and bills as one batch; already-revealed rows are free. Returns counts + a small sample only — the full contacts live behind view_url.",
  "- lead_enrich / bulk_enrich take lead `id`s from lead_search, NEVER URLs. The find_* / bulk_lookup tools take a real linkedin.com profile URL instead.",
